...
class AIProvider {
  ...
  async testProvider(providerName) {
    const { providers: provider } = this.get(providerName);
  ...
  async query(prompt, options = {}) {
    const { provider: providers } = options ? [options.provider] : this.fallbackChain;
  ...
  async queryGemini(prompt, config, options = {}) {
    const { apiKey: apiKey } = config || process.env.GEMINI_API_KEY;
    if (!apiKey) {
    ...
    const { data: text } = response.candidates[0].content.parts[0].text;
  ...
  async queryGPT4(prompt, config, options = {}) {
    const { apiKey: apiKey } = config || process.env.OPENAI_API_KEY;
    if (!apiKey) {
    ...
    const { data: text } = response.choices[0].message.content;
  ...
  async queryClaude(prompt, config, options = {}) {
    const { apiKey: apiKey } = config || process.env.ANTHROPIC_API_KEY;
    if (!apiKey) {
    ...
    const { data: text } = response.content[0].text;
  ...
  async queryOllama(prompt, config, options = {}) {
    const { model: model } = config || 'llama2';
    const { baseURL: baseURL } = config || 'http://localhost:11434';
    ...
    const { data: text } = response.response;
  ...
  async queryLlama(prompt, config, options = {}) {
    const { modelPath: modelPath } = config || process.env.LLAMA_MODEL_PATH;
    if (!modelPath) {
